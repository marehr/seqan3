...
struct target_migratable<std::vector<value_t>, mode>
{
    ...
    target_migratable(node_t target_node, std::vector<value_t> const & vector)
        : target_migratable{}
    {
        // std::cout << "target_migratable<target_node, vector>-ctor on " << ham::offload::this_node() << " -> " << target_node << ": "; debug_stream << vector; std::cout << std::endl;
    ...
    target_migratable(std::vector<value_t> vector)
        : target_migratable{}
    {
        // std::cout << "target_migratable<vector>-ctor on " << ham::offload::this_node() << " -> " /*<< target_node*/ << ": "; debug_stream << vector; std::cout << std::endl;
        static_assert(!contiguous_container_t::is_owning(), "contiguous_container_t is non-owning");
