...
// simd_t is a pointer to a incomplete type. Furthermore the is_pointer_v and is_array_v should prevent those cases by
// checking the type beforehand, but for some reasons the short-circuit semantic of `&&` does not work in this case and
// gcc still evaluates the requires clause which in turn triggers the error.
//
...
//!\cond
template <typename simd_t>
SEQAN3_CONCEPT simd_concept = !std::is_pointer_v<simd_t> && !std::is_array_v<simd_t> && detail::simd_concept<simd_t>;
//!\endcond
